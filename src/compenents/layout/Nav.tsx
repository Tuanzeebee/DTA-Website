...
export function Nav({ lang, toggleLang, isLoggedIn }: NavProps) {
  ...
  return (
    <>
      ...
      {/* Rendered as a SIBLING of the header, not a child: the header carries
          `backdrop-filter` (via .glass) whenever it is scrolled or the menu is
          open, and a backdrop-filtered element becomes the containing block for
          its fixed descendants. Nested inside the header, the sheet's
          `top-16 bottom-0` would resolve against the 64px header box and
          collapse to zero height — so the menu opened but showed nothing. */}
      <MobileNavSheet
        open={menuOpen}
